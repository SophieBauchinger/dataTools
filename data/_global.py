...
# %% Global data
class GlobalData(SelectionMixin, TropopauseSorterMixin, AnalysisMixin):
    ...
    def pickle_data(self, fname: str=None, pdir=None, special=None):
        """ Save data dictionary using dill. Fname should be 'id_DATA(special)_date.pkl' """
        if not fname: 
            date_today = dt.datetime.now().strftime("%y%m%d")
            fname = f'{self.ID.lower()}_DATA'+ (special if special else '')+f'_{date_today}' +'.pkl'
...
# --- Instance variables (substances / coordinates) ---
    def get_variables(self, category):
        ...
        # print(f"Did not find valid {} for {not_found}")
        # TODO: How to deal with substances / coordiantes that are not represented? 
        return variables
...
# --- Define additional attributes ---
    ...
    def flights(self):
        ...
        fl_cols = [c for c in self.df.columns if 'flight' in c.lower()]
...
# TODO: Decide if this is sensible or not neccessary
class SondeData(GlobalData):
    """ Idea of creating a subclass of Global data for just Sonde measurements (e.g. WOUDC). """
