...
# Bin2dPlotter(tpause.df).plot_2d_stdv(
...
#TODO make sure x and y are actually correct when binning and plotting
...
class Bin2dPlotter():
    ...
    def get_limits(self, subs, xcoord, ycoord) -> (tuple, tuple, tuple):
        ...
        if 'vlims' in self.kwargs:
            ...
            try: vlims = dcts.get_default_vlims(subs.short_name)
...
{dcts.make_subs_label(subs2)} {dcts.make_coord_label(xcoord2)} {dcts.make_coord_label(ycoord2)}')
    ...
    def plot_2d_stdv(self, subs, xcoord, ycoord, **kwargs):
        ...
        cmap = cmr.get_sub_cmap('YlOrBr', 0.3, 1)
        ...
        for season, ax in zip([1,2,3,4], axs.flatten()):
            ...
            ax.set_yticks(np.arange(bci.ybmin, bci.ybmax + bci.ybsize,  bci.ybsize*3))
            ...
            if ycoord.rel_to_tp is True and ycoord.vcoord in ['pt', 'z']: 
                ...
                tropo_weighted_average =  np.average(tropo_stdv, weights = tropo_weights)
                ...
                strato_weighted_average =  np.average(strato_stdv, weights = strato_weights)
    
    
                ax.text(**dcts.note_dict(ax, x=0.25, y = 0.9, 
